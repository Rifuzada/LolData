        ...
        if (response.config.params.riotId != "") {
          ...
          axios.get('http://localhost:4000/profile', { params: { puuid, region } })
            .then(response => {
              ...
              axios.get('http://localhost:4000/ranked', { params: { sumID, region } })
                .then(ranked => {
                  if (ranked.data[0] == undefined) {
                  ...
                  if (ranked.data[1] == undefined) {
                  ...
                  else if (ranked.data[0].queueType == "CHERRY") {
                    delete ranked.data[0]
                  }
                  else if (ranked.data[0].queueType == "RANKED_FLEX_SR" && ranked.data[1].queueType == "RANKED_SOLO_5x5") {
                    var queueFlex = ranked.data[0].queueType
                    queueFlex = ranked.data[0].queueType.replace('RANKED_FLEX_SR', 'Ranqueada Flexiviel: ')
                      .replace('RANKED_SOLO_5x5', 'Ranqueada Solo/Duo: ');
                    ranked.data[0].tier = ranked.data[0].tier.toLowerCase()
                    ...
                    queueSolo = ranked.data[1].queueType.replace('RANKED_FLEX_SR', 'Ranqueada Flexiviel: ')
                      .replace('RANKED_SOLO_5x5', 'Ranqueada Solo/Duo: ');
                    ranked.data[1].tier = ranked.data[1].tier.toLowerCase()
                  ...
                  else if (ranked.data[0].queueType == "RANKED_SOLO_5x5" && ranked.data[1].queueType == "RANKED_FLEX_SR") {
                    console.log("aqui")
                    var queueSolo = ranked.data[0].queueType
                    queueSolo = ranked.data[0].queueType.replace('RANKED_FLEX_SR', 'Ranqueada Flexiviel: ')
                      .replace('RANKED_SOLO_5x5', 'Ranqueada Solo/Duo: ');
                    ranked.data[0].tier = ranked.data[0].tier.toLowerCase()
                    ...
                    queueFlex = ranked.data[1].queueType.replace('RANKED_FLEX_SR', 'Ranqueada Flexiviel: ')
                    ranked.data[1].tier = ranked.data[1].tier.toLowerCase()
                  ...
                  else if (ranked.data[1].queueType == "RANKED_SOLO_5x5") {
                    var queueSolo = ranked.data[1].queueType
                    rankedFlexIcon.style.display = ("none")
                    containerRankedFlex.style.display = ("none")
                    queueSolo = ranked.data[1].queueType.replace('RANKED_FLEX_SR', 'Ranqueada Flexiviel: ')
                      .replace('RANKED_SOLO_5x5', 'Ranqueada Solo/Duo: ');
                    ranked.data[1].tier = ranked.data[1].tier.toLowerCase()
                    rankedSoloIcon.src = `https://raw.communitydragon.org/latest/plugins/rcp-fe-lol-static-assets/global/default/images/ranked-mini-crests/${ranked.data[1].tier}.svg`
                  ...
                  else if (ranked.data[0].queueType == "RANKED_SOLO_5x5") {
                    var queueSolo = ranked.data[0].queueType
                    rankedFlexIcon.style.display = ("none")
                    containerRankedFlex.style.display = ("none")
                    queueSolo = ranked.data[0].queueType.replace('RANKED_FLEX_SR', 'Ranqueada Flexiviel: ')
                  ...
                  else if (ranked.data[0].queueType == "RANKED_FLEX_SR") {
                    var queueFlex = ranked.data[0].queueType
                  ...
                  else if (ranked.data[1].queueType == "RANKED_FLEX_SR") {
                    rankedSoloIcon.style.display = ("none")
                    containerRanked.style.display = ("none")
                    queueFlex = ranked.data[1].queueType.replace('RANKED_FLEX_SR', 'Ranqueada Flexiviel: ')
                    ranked.data[1].tier = ranked.data[1].tier.toLowerCase()
