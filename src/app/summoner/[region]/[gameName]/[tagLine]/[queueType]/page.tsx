...
export default async function SummonerPage({ params }: SummonerPageProps) {
  ...
  try {
    ...
    const [queueTypes, masteries, matchesByQueue, rankedData] =
      await Promise.all([
        getQueueTypes(),
    ...
    // Compute last 20 games stats
    const last20Matches = (matchesByQueue as any[]).slice(0, 20);

    const lastWins = last20Matches.filter((match) => {
          ...
          >
            <header className="mb-6">
              ...
              <p className="text-sm text-muted-foreground">
                Recent games played by {decodedGameName} on {queueType} with all
                champions - last {lastWins + lastLoses} games: {lastWins} -{" "}
